...
const syncConversationOwner = async (conversationId, assignedToUserId) => {
...
export const listPlatformTasks = asyncHandler(async (req, res) => {
  ...
  const tasks = await SupportTask.find(query)
    ...
    .populate("conversationId", "subject status priority assignedToUserId")
...
export const createConversationTask = asyncHandler(async (req, res) => {
  ...
  if (req.body.assignedToUserId !== undefined) {
    await syncConversationOwner(conversation._id, requestedAssignee);
  }
...
export const updateConversationTask = asyncHandler(async (req, res) => {
  ...
  const assigneeWasUpdated = req.body.assignedToUserId !== undefined;
  if (assigneeWasUpdated) {
  ...
  await task.save();
  if (assigneeWasUpdated) {
    await syncConversationOwner(task.conversationId, normalizedAssignee);
  }
  await task.populate("assignedToUserId", "name email platformRole");
