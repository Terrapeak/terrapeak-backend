...
const PASSWORD_RESET_WORDS = /\b(password reset|reset password|forgot password|password link|reset link)\b/i;
const EMAIL_PATTERN = /\b[A-Z0-9._%+-]+@[A-Z0-9.-]+\.[A-Z]{2,}\b/i;
const ACTION_TTL_MS = 15 * 60 * 1000;
const ADMIN_ROLES = new Set(["owner", "admin"]);
const ACTIVITY_LIMIT = 50;

const assistantResult = (body, { completed = false } = {}) => ({
  handled: true,
  body,
  completed,
});
...
const executePendingAction = async ({ conversation, requester }) => {
  ...
  if (String(pending.requestedByUserId) !== String(requester._id)) {
    return assistantResult("This pending action can only be confirmed by the person who requested it.");
  }
  ...
  const membership = await CompanyMembership.findOne({
    _id: pending.membershipId,
    companyId: conversation.companyId,
    userId: pending.targetUserId,
    isActive: true,
  });
  const [company, targetUser] = await Promise.all([
    Company.findById(conversation.companyId),
    User.findById(pending.targetUserId),
  ]);
  ...
  if (pending.type === "resend_invitation") {
    ...
    await appendActivity({
      ...
      title: "Company invitation resent through support automation",
      requester,
      targetUser,
    });
    conversation.pendingAction = null;
  ...
  await appendActivity({
    companyId: company._id,
    title: "Password reset email sent through support automation",
    requester,
    targetUser,
  });
  conversation.pendingAction = null;
...
export const handleSupportAccountAction = async ({ conversation, requesterMembership, requester, body }) => {
  ...
  if (conversation.pendingAction) {
    if (CANCEL_WORDS.test(request)) {
      if (String(conversation.pendingAction.requestedByUserId) !== String(requester._id)) {
        return assistantResult("This pending action can only be cancelled by the person who requested it.");
      }
      conversation.pendingAction = null;
      await conversation.save();
  ...
  const actionType = PASSWORD_RESET_WORDS.test(request)
    ? "password_reset"
    : RESEND_INVITE_WORDS.test(request)
      ? "resend_invitation"
      : null;
  ...
  if (!targetEmail) {
    return assistantResult("Please provide the email address of the company user whose invitation should be resent.");
  }
  ...
  if (!targetMembership?.userId) {
    return assistantResult(`No active company user was found with the email address ${targetEmail}. No action was performed.`);
  }
  ...
  if (!isSelf && !canManageOthers) {
    return assistantResult("Only a company owner or administrator can request this action for another user.");
  }
  if (actionType === "resend_invitation" && !canManageOthers) {
    return assistantResult("Only a company owner or administrator can resend company invitations.");
  }
  if (actionType === "resend_invitation" && targetMembership.userId.invitationStatus === "accepted") {
    return assistantResult(`${targetEmail} has already accepted the invitation. A password reset may be more appropriate if access is the issue.`);
  }

  conversation.pendingAction = {
    type: actionType,
    membershipId: targetMembership._id,
    targetUserId: targetMembership.userId._id,
    targetEmail,
  ...
  const actionLabel = actionType === "password_reset"
    ? "send a password-reset email"
    : "resend the company invitation";
