...
const extractValue = (text, field) => {
  ...
  if (field === "website") {
    return text.match(/https?:\/\/\S+|www\.\S+/i)?.[0]?.replace(/[.,]+$/, "") || "";
  }
  if (field === "phone") {
    const phone = text.match(/\+?\d[\d\s().-]{5,30}\d/)?.[0];
...
export const handleSupportCompanyUpdate = async ({ conversation, requesterMembership, requester, body }) => {
  ...
  if (oldValue === newValue) {
    return result(`The ${FIELD_LABELS[field]} is already set to ${newValue}. No change is needed.`);
  }
