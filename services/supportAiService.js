...
const mapGeminiError = (status, payload) => {
  ...
  if (status === 400 && rawMessage.includes("api key")) {
    return createSupportAiError("INVALID_API_KEY", "Gemini rejected the API key. Replace GEMINI_API_KEY with a valid Google AI Studio key.", 400);
  }
  if (status === 401 || status === 403) {
    return createSupportAiError("PERMISSION_DENIED", "Gemini denied access. Check the API key restrictions and whether the Generative Language API is enabled.", status);
  }
  if (status === 404 || rawMessage.includes("not found") || rawMessage.includes("model")) {
    return createSupportAiError("MODEL_UNAVAILABLE", `The support model ${SUPPORT_MODEL} is unavailable for this Gemini key.`, 400);
  }
  if (status === 429 || rawMessage.includes("quota") || rawMessage.includes("rate limit")) {
    return createSupportAiError("QUOTA_EXCEEDED", "Gemini quota or rate limit has been reached. Check the Google AI project quota and billing.", 429);
  }
...
export const analyzeSupportConversation = async ({ subject, messages }) => {
  if (!process.env.GEMINI_API_KEY) {
    throw createSupportAiError("MISSING_API_KEY", "GEMINI_API_KEY is not configured on the backend.", 503);
  }
...
${transcript}`;
  ...
  try {
    ...
    if (!analysis) {
      throw createSupportAiError("INVALID_AI_RESPONSE", "Gemini returned a response that could not be parsed as support analysis.", 502);
    }
    ...
    if (error?.name === "AbortError") {
      throw createSupportAiError("AI_TIMEOUT", "Gemini analysis timed out after 12 seconds.", 504);
    }
    if (error?.code) throw error;
    throw createSupportAiError("AI_NETWORK_ERROR", "The backend could not reach Gemini.", 503);
  } finally {
