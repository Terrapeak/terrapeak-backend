...
const validId = (id, code = "AVATAR_NOT_FOUND") => { if (!mongoose.Types.ObjectId.isValid(id)) throw avatarServiceError(code, code === "AVATAR_VIDEO_NOT_FOUND" ? "Avatar video not found." : "Avatar not found.", 404); return id; };
...
const readiness = ({ avatar, selected, available, authorized }) => {
  const reasons = [];
...
export const acceptAvatarConsent = async ({ companyId, userId, body, acceptedIp }) => {
  ...
  if (!existing || wasRevoked) Object.assign($set, { status: "not_started", selectedAvatarId: null, approvedAt: null, approvedVideoId: null });
...
export const refreshAvatarVideo = async ({ companyId, userId, videoId, provider: injectedProvider, copyVideo = copyProviderVideoToPrivateStorage, deleteVideo = deletePrivateAvatarVideo }) => {
  ...
  const setup = await assertAvatarConsent({ companyId, userId }); if (String(setup.selectedAvatarId) !== String(video.avatarId)) throw avatarServiceError("AVATAR_AUTHORIZATION_CHANGED", "Avatar authorization changed during generation.", 409);
...
export const getAvatarState = async ({ companyId, userId, provider: injectedProvider }) => {
  ...
  const selected = candidates.find((candidate) => String(candidate._id) === String(setup?.selectedAvatarId)); const videos = authorized ? await DigitalCloneAvatarVideo.find({ companyId, userId }).sort({ createdAt: -1 }).limit(20) : [];
  ...
  return { avatar: { status: setup?.status || "not_started", consent: setup?.consent || null, selectedAvatarId: authorized ? setup?.selectedAvatarId || null : null, approvedAt: authorized ? setup?.approvedAt || null : null, readiness: readiness({ avatar: setup, selected, available: availability.available, authorized }) }, providerAvailability: { available: availability.available, ...(!availability.available ? { code: availability.code } : {}) }, availableAvatars: candidates.map(serializeCandidate), videos: videos.map(serializeAvatarVideo), approvedDrafts: drafts.map((draft) => ({ id: draft._id, topic: draft.topic, text: draft.finalApprovedText, approvedAt: draft.approvedAt })), limits: { maxScriptCharacters: MAX_SCRIPT_CHARACTERS, generationPerHour: 3 } };
};
