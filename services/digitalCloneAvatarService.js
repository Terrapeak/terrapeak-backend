...
const VIDEO_FIELDS = new Set(["sourceType", "approvedDraftId", "draftId", "script", "aspectRatio", "resolution", "captions", "background"]);
const VIDEO_SOURCE_TYPES = new Map([["approved_draft", "approved-draft"], ["manual_test", "manual-test"], ["approved-draft", "approved-draft"], ["manual-test", "manual-test"]]);
const serializeVideoSourceType = (sourceType) => sourceType === "approved-draft" ? "approved_draft" : sourceType === "manual-test" ? "manual_test" : sourceType;
...
export const serializeAvatarVideo = (video) => { const value = video?.toObject ? video.toObject() : video; return { id: value._id, avatarId: value.avatarId, sourceDraftId: value.sourceDraftId, sourceType: serializeVideoSourceType(value.sourceType), purpose: value.purpose || "avatar-validation", scriptSnapshot: value.scriptSnapshot, aspectRatio: value.aspectRatio, resolution: value.resolution, captions: value.captions, background: value.background, status: value.status, failureCode: value.failureCode, durationSeconds: value.durationSeconds, approvedAt: value.approvedAt, createdAt: value.createdAt, updatedAt: value.updatedAt, deliveryPath: ["completed", "approved"].includes(value.status) ? `/digital-clone/avatar/videos/${value._id}/delivery` : null }; };
const approvedVideoMatchesCurrentConfiguration = ({ avatar, approvedVideo }) => Boolean(
  avatar?.approvedAt && avatar?.approvedVideoId && approvedVideo?.approvedAt && approvedVideo?.status === "approved"
  && String(approvedVideo._id) === String(avatar.approvedVideoId)
  && String(approvedVideo.avatarId) === String(avatar.selectedAvatarId)
  && String(approvedVideo.providerVoiceId) === String(avatar.selectedProviderVoiceId)
);
const readiness = ({ avatar, selected, selectedVoice, approvedVideo, available, authorized }) => {
  const reasons = [];
  ...
  if (!approvedVideoMatchesCurrentConfiguration({ avatar, approvedVideo })) reasons.push("human-approval-required"); if (avatar?.status === "revoked") reasons.push("revoked");
...
const normalizeVideoInput = async ({ companyId, userId, body }) => {
  strictObject(body, VIDEO_FIELDS, "AVATAR_VIDEO_INVALID"); const sourceType = VIDEO_SOURCE_TYPES.get(body.sourceType);
  if (!sourceType) throw avatarServiceError("AVATAR_VIDEO_INVALID", "Select an approved draft or manual test script.");
  let script; let sourceDraftId = null;
  if (sourceType === "approved-draft") {
    const legacySource = body.sourceType === "approved-draft"; const approvedDraftId = legacySource ? body.draftId : body.approvedDraftId;
    if (Object.hasOwn(body, "script") || Object.hasOwn(body, legacySource ? "approvedDraftId" : "draftId")) throw avatarServiceError("AVATAR_VIDEO_INVALID", "Approved draft requests cannot include ambiguous source fields.");
    validId(approvedDraftId, "DRAFT_NOT_FOUND");
    const draft = await DigitalCloneGeneration.findOne({ _id: approvedDraftId, companyId, userId, contentType: "short-video-script", status: "approved" }).lean();
    if (!draft?.finalApprovedText) throw avatarServiceError("DRAFT_NOT_APPROVED", "Select an approved short-video-script draft.", 409);
    script = draft.finalApprovedText.trim(); sourceDraftId = draft._id;
  } else {
    if (Object.hasOwn(body, "approvedDraftId") || Object.hasOwn(body, "draftId") || typeof body.script !== "string") throw avatarServiceError("AVATAR_VIDEO_INVALID", "Manual test requests require only a test script.");
    script = body.script.trim();
  }
  if (!script) throw avatarServiceError("AVATAR_SCRIPT_REQUIRED", "Avatar scripts must contain at least one non-whitespace character.");
  if (script.length > MAX_SCRIPT_CHARACTERS) throw avatarServiceError("AVATAR_SCRIPT_TOO_LONG", `Avatar scripts must contain at most ${MAX_SCRIPT_CHARACTERS} characters.`);
...
export const createAvatarVideo = async ({ companyId, userId, body, provider: injectedProvider, purpose = "avatar-validation", testCloneId = null }) => {
  if (!["avatar-validation", "test-clone"].includes(purpose)) throw avatarServiceError("AVATAR_VIDEO_INVALID", "Avatar video purpose is invalid.");
  ...
  const dedupeKey = createHash("sha256").update(JSON.stringify({ purpose, testCloneId: String(testCloneId || ""), avatar: String(candidate._id), voice: String(voice._id), ...input })).digest("hex"); let video;
  try { video = await DigitalCloneAvatarVideo.create({ ...owned({ companyId, userId }), avatarId: candidate._id, providerVoiceId: voice._id, purpose, testCloneId, sourceDraftId: input.sourceDraftId, sourceType: input.sourceType, scriptSnapshot: input.scriptSnapshot, aspectRatio: input.aspectRatio, resolution: input.resolution, captions: input.captions, background: input.background, status: "queued", dedupeKey, activeDedupeKey: dedupeKey }); }
...
export const approveAvatarVideo = async ({ companyId, userId, videoId }) => {
  const setup = await assertAvatarConsent({ companyId, userId }); const video = await findVideo({ companyId, userId, videoId, privateFields: true });
  if (video.purpose === "test-clone") throw avatarServiceError("AVATAR_VIDEO_NOT_APPROVABLE", "Complete and review an Avatar validation video.", 409);
  const [selected, selectedVoice] = await Promise.all([
    DigitalCloneAvatarCandidate.findOne({ _id: setup.selectedAvatarId, companyId, userId, status: "selected", providerReady: true }),
    DigitalCloneAvatarProviderVoice.findOne({ _id: setup.selectedProviderVoiceId, companyId, userId, status: "selected", providerReady: true }),
  ]);
  if (video.status !== "completed" || !video.storagePublicId || !selected || !selectedVoice || String(video.avatarId) !== String(selected._id) || String(video.providerVoiceId) !== String(selectedVoice._id)) throw avatarServiceError("AVATAR_VIDEO_NOT_APPROVABLE", "Complete and review a test video for the current Avatar and voice.", 409);
  const approvedAt = new Date(); await DigitalCloneAvatarVideo.updateMany({ companyId, userId, status: "approved", _id: { $ne: video._id } }, { $set: { status: "completed" } });
  const approved = await DigitalCloneAvatarVideo.findOneAndUpdate({ _id: video._id, companyId, userId, status: "completed", purpose: { $ne: "test-clone" }, avatarId: selected._id, providerVoiceId: selectedVoice._id }, { $set: { status: "approved", approvedAt } }, { new: true, runValidators: true });
  if (!approved) throw avatarServiceError("AVATAR_VIDEO_NOT_APPROVABLE", "Complete and review a test video for the current Avatar and voice.", 409);
  const readySetup = await DigitalCloneAvatar.findOneAndUpdate({ _id: setup._id, companyId, userId, status: { $ne: "revoked" }, selectedAvatarId: video.avatarId, selectedProviderVoiceId: video.providerVoiceId, "consent.revokedAt": null }, { $set: { status: "ready", approvedAt, approvedVideoId: video._id } }, { new: true, runValidators: true });
  if (!readySetup) { await DigitalCloneAvatarVideo.updateOne({ _id: video._id, companyId, userId, status: "approved" }, { $set: { status: "archived", approvedAt: null, activeDedupeKey: null } }); throw avatarServiceError("AVATAR_AUTHORIZATION_CHANGED", "Avatar authorization changed during approval.", 409); }
  return approved;
};
export const rejectAvatarVideo = async ({ companyId, userId, videoId, purpose = "avatar-validation" }) => { await assertAvatarConsent({ companyId, userId }); validId(videoId, "AVATAR_VIDEO_NOT_FOUND"); const purposeFilter = purpose === "test-clone" ? { purpose: "test-clone" } : { purpose: { $ne: "test-clone" } }; const video = await DigitalCloneAvatarVideo.findOneAndUpdate({ _id: videoId, companyId, userId, ...purposeFilter, status: { $in: ["completed", "failed"] } }, { $set: { status: "rejected", rejectedAt: new Date(), activeDedupeKey: null } }, { new: true }); if (!video) throw avatarServiceError("AVATAR_VIDEO_NOT_REJECTABLE", "This avatar video cannot be rejected.", 409); return video; };
...
export const getAvatarState = async ({ companyId, userId, provider: injectedProvider }) => {
  ...
  const selected = candidates.find((candidate) => String(candidate._id) === String(setup?.selectedAvatarId)); const selectedVoice = providerVoices.find((voice) => String(voice._id) === String(setup?.selectedProviderVoiceId)); const videos = authorized ? await DigitalCloneAvatarVideo.find({ companyId, userId, purpose: { $ne: "test-clone" } }).sort({ createdAt: -1 }).limit(20) : [];
  const approvedVideo = authorized && setup?.approvedVideoId ? await DigitalCloneAvatarVideo.findOne({ _id: setup.approvedVideoId, companyId, userId, status: "approved", purpose: { $ne: "test-clone" } }).select("avatarId providerVoiceId status approvedAt").lean() : null;
  const avatarReadiness = readiness({ avatar: setup, selected, selectedVoice, approvedVideo, available: availability.available, authorized });
  const drafts = (await DigitalCloneGeneration.find({ companyId, userId, contentType: "short-video-script", status: "approved" }).select("topic finalApprovedText approvedAt").sort({ approvedAt: -1 }).limit(50).lean())
    .filter((draft) => typeof draft.finalApprovedText === "string" && draft.finalApprovedText.trim().length > 0 && draft.finalApprovedText.trim().length <= MAX_SCRIPT_CHARACTERS)
    .slice(0, 20);
  return { avatar: { status: avatarReadiness.ready ? "ready" : setup?.status === "ready" ? "selected" : setup?.status || "not_started", consent: setup?.consent || null, selectedAvatarId: authorized ? setup?.selectedAvatarId || null : null, selectedProviderVoiceId: authorized ? setup?.selectedProviderVoiceId || null : null, approvedAt: avatarReadiness.ready ? approvedVideo.approvedAt : null, approvedVideoId: avatarReadiness.ready ? approvedVideo._id : null, readiness: avatarReadiness }, providerAvailability: { available: availability.available, ...(!availability.available ? { code: availability.code } : {}) }, availableAvatars: candidates.map(serializeCandidate), availableProviderVoices: providerVoices.map(serializeProviderVoice), videos: videos.map(serializeAvatarVideo), approvedDrafts: drafts.map((draft) => ({ id: draft._id, topic: draft.topic, text: draft.finalApprovedText, approvedAt: draft.approvedAt })), limits: { maxScriptCharacters: MAX_SCRIPT_CHARACTERS, generationPerHour: 3 } };
};
