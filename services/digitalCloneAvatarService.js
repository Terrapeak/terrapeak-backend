...
export const serializeAvatarVideo = (video) => { const value = video?.toObject ? video.toObject() : video; return { id: value._id, avatarId: value.avatarId, sourceDraftId: value.sourceDraftId, sourceType: serializeVideoSourceType(value.sourceType), scriptSnapshot: value.scriptSnapshot, aspectRatio: value.aspectRatio, resolution: value.resolution, captions: value.captions, background: value.background, status: value.status, failureCode: value.failureCode, durationSeconds: value.durationSeconds, approvedAt: value.approvedAt, createdAt: value.createdAt, updatedAt: value.updatedAt, deliveryPath: ["completed", "approved"].includes(value.status) ? `/digital-clone/avatar/videos/${value._id}/delivery` : null }; };
...
export const approveAvatarVideo = async ({ companyId, userId, videoId }) => {
  ...
  const approved = await DigitalCloneAvatarVideo.findOneAndUpdate({ _id: video._id, companyId, userId, status: "completed", avatarId: selected._id, providerVoiceId: selectedVoice._id }, { $set: { status: "approved", approvedAt } }, { new: true, runValidators: true });
...
export const getAvatarState = async ({ companyId, userId, provider: injectedProvider }) => {
  ...
  const approvedVideo = authorized && setup?.approvedVideoId ? await DigitalCloneAvatarVideo.findOne({ _id: setup.approvedVideoId, companyId, userId, status: "approved" }).select("avatarId providerVoiceId status approvedAt").lean() : null;
